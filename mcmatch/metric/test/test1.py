...
'''
import unittest
from mcmatch.metric.counter import CallCounter, MultiMnemonicCounterMetric
...
class CallCounterTest(unittest.TestCase):
    ...
    def testCallCounter(self):
        ...
        self.assertEqual(cc.get_sql_table(), 'metric_mncounter_call')
        
        sql_chunk_start = "SELECT function_text_id"
        sql_chunk_signature = ", function_text.signature"
        sql_chunk_metrics = ", metric_mncounter_call.mn_call FROM metric_mncounter_call"
        ...
        self.assertEqual(standard_sql_select[0], sql_chunk_start + sql_chunk_metrics)
        self.assertEqual(len(standard_sql_select[1]), 0)
        
        sql_select_with_signature = cc.get_sql_select(None, True)
        self.assertEqual(sql_select_with_signature[0], sql_chunk_start + sql_chunk_signature + sql_chunk_metrics + sql_chunk_ft_join)
        ...
        self.assertEqual(sql_select_in_repositories_simplewhitespace,
                         sql_chunk_start + sql_chunk_signature + sql_chunk_metrics + sql_chunk_ft_join + sql_chunk_repo)

    def _generic_precheck(self, cc, is_relative=False):
      self.assertEqual(len(cc.get_sql_contents()), len(cc.get_sql_columns()))
      self.assertEqual(len(cc.get_sql_contents()), cc.num_metrics())
      self.assertEqual(cc.get_sql_contents(), [None for x in range(cc.num_metrics())])
